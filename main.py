...
# ================= 🔐 КОНФИГУРАЦИЯ =================
# 🔥 Фоллбэк токен (если переменная окружения не установлена)
TOKEN = os.getenv("BOT_TOKEN", "")

HLTV_URL = "https://www.hltv.org"
USE_PROXY = False  # На сервере прокси не нужен
...
async def fetch_hltv(url):
    ...
    try:
        ...
        async with aiohttp.ClientSession(headers=HEADERS, connector=connector) as session:
            kwargs = {'proxy': PROXY_URL} if USE_PROXY else {}
...
async def get_upcoming_matches():
    ...
    if not soup:
        return "❌ Не удалось загрузить матчи"
...
async def cmd_start(message: types.Message):
    await message.answer("👋 <b>CS2 Stats Bot</b>\n\nВыбери 👇", reply_markup=get_keyboard(), parse_mode="HTML")
...
async def ask_team(message: types.Message, state: FSMContext):
    await message.answer("✍️ <b>Напиши команду:</b>\n\nПримеры: NAVI, FaZe, Vitality\n\nИли /cancel", parse_mode="HTML", reply_markup=ReplyKeyboardRemove())
